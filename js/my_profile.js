...
function updateAdminDetails() {
  ...
  var email = document.getElementById('email'); // Fixed typo
  ...
  if (!validateName(pharmacy_name.value, 'pharmacy_name_error'))
    ...
    if (validatePassword(password)) {
      ...
      xhttp.onreadystatechange = function () {
        if (xhttp.readyState == 4 && xhttp.status == 200) { // Fixed comparison
          document.getElementById('admin_acknowledgement').innerHTML = xhttp.responseText;
          setTimeout(() => edit(true), 1500); // Let user see the message
        }
      };
      xhttp.open("GET", "php/validateCredentials.php?action=update_admin_info&pharmacy_name=" + encodeURIComponent(pharmacy_name.value)
        + "&address=" + encodeURIComponent(address.value)
        + "&email=" + encodeURIComponent(email.value)
        + "&contact_number=" + encodeURIComponent(contact_number.value)
        + "&username=" + encodeURIComponent(username.value), true);
      xhttp.send();
      return true;
    } else {
...
function validatePassword(password) {
  ...
  xhttp.send();
  return xhttp.responseText == "true"; // Fixed logic and removed broken callback
}
...
function changePassword() {
  ...
  if (!checkAdminPassword(old_password.value, 'old_password_error'))
    ...
    xhttp.open("GET", "php/validateCredentials.php?action=change_password&password=" + encodeURIComponent(password.value), false);
    xhttp.send();
    return true;
  }
  return false;
