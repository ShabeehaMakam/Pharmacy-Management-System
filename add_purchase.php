<?php
$conn = new mysqli("localhost", "root", "", "pharmacy"); // Replace with your actual DB name
...
if (isset($_GET['ajax'])) {
  ...
  if ($_GET['ajax'] == 'invoice') {
    $result = $conn->query("SELECT MAX(INVOICE_ID) AS last_invoice FROM invoices");
    $row = $result->fetch_assoc();
    $nextInvoice = $row['last_invoice'] ? $row['last_invoice'] + 1 : 1001;
    echo json_encode(['invoice' => $nextInvoice]);
    exit;
  }
}
?>
...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <script>
      // Auto-fill invoice number on page load
$.getJSON("?ajax=invoice", function (data) {
  $('#invoice_number').val(data.invoice);
});
